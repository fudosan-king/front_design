...
function log_mainsion() {
    ...
    this.scrollToEle = function () {
        ...
        $("body").on("click", function (e) {
            if (e.target.hash && e.target.hash.includes('#')) {
                //turn off menu
                _this.hamburgerMenu();
                const anchor = document.getElementById(e.target.hash.slice(1)),
                    targetToTop = $(anchor).offset().top ? $(anchor).offset().top : 0;
                const targetHeight = $(anchor).outerHeight();
    ...
    this.animationNav = function () {
        const _this = this;
        $('button[data-open-hamburger]').on('click', function (e) {
            _this.hamburgerMenu();
        });
    }

    this.hamburgerMenu = function () {
        if ($('header').hasClass('open-hamburger')) {
            $('.nav-menu-list').removeClass('show');
            $('header').removeClass('open-hamburger');
            $('li[js-animation]').removeAttr('style');
        } else {
            $('.nav-menu-list').addClass('show');
            $('li[js-animation]').each(function (i, el) {
                $(el).css({ transition: "opacity .2s cubic-bezier(.420, .000, .580, 1.000) " + (0.2 + 0.07 * (i + 1)) + "s, transform .2s cubic-bezier(.420, .000, .580, 1.000) " + (0.2 + 0.07 * (i + 1)) + "s" });
            });
            setTimeout(function () {
                $('header').addClass('open-hamburger');
            }, 100);
        }
    }
            ...
            if ($(window).width() <= 991) {
                if ($('.detail_des').length > 0 && $('body .view-more').length === 0) {
                    if (current_height > your_height) {
                        ...
                        wrap.after(function () {
                            return '<div class="view-more"><span class="i-open" title="Xem thﾃｪm" ></span></div>';
                        });
                        //wrap.after(function(){
                        //return '<div class="view-hide" style="display: none"><span class="i-close" title="Thu g盻肱"></span></div>';
                        //});
                    }
                }
            } else {
