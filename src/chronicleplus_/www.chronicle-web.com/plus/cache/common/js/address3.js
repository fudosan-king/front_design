...
(function($, window) {
  ...
  // 自分のURLを取得する
  var SCRIPT_HOST = 'https://fudosan-king.jp';
  ...
  AddressSelector.prototype = {
    initialize_address_selector: function() {
      var self = this;
      this.$selects = this.$fields.find('.pref, .city');
      ...
      $('.lines').on('change', function() {
        self.set_stations(this);
      });
      var pref_data = $('.pref-data').val();
      if (pref_data) {
        var data_city = {};
        $.ajax(SCRIPT_HOST + '/api/area/' + pref_data, {
          cache: true,
          dataType: 'jsonp'
        })
          .done(function(data) {
            self.update_select_list(1, data.areas);
          })
          .fail(function() {
            // TODO
          });
    ...
    update_select_list: function(index, choices) {
      ...
      $(choices).each(function(idx, val) {
        ...
        if(!val)
          text = '(なし)';
        if (val == pref_data || val == city_data) {
          $('<option>').val(val).text(text).attr("selected", true).appendTo($target);
        } else {
    ...
    update_lines: function(choices) {
      var $target = $('.station-fields .lines');

      $target.find('option:not(:first)').remove();
      $target.prop('disabled', false);
      $(choices).each(function(idx, dict) {
        for (var optgroup in dict) {
            $('<optgroup label="' + optgroup + '">').appendTo($target);
              for (var idx in dict[optgroup]){
                $('<option>').val(dict[optgroup][idx]).text(dict[optgroup][idx]).appendTo($target);
              }
    ...
    update_stations: function(choices) {
      var $target = $('.station-fields .stations');
      $target.find('option:not(:first)').remove();
      $target.prop('disabled', false);
      $(choices).each(function(idx, val) {
        $('<option>').val(val['name']).text(val['name']).appendTo($target);
      });
    },

    set_stations: function(changed) {
      var self = this;
      ...
      var pref = $('.pref').val();
      var lines = $('.lines').val();
      $.ajax(SCRIPT_HOST + '/api/pref/' + pref + '/' + lines + '/stations', {
        cache: true,
        dataType: 'jsonp'
      })
        .done(function(data) {
          self.update_stations(data.stations);
        })
        .fail(function() {
          // TODO
        });
    ...
    set_lines: function(changed) {
      ...
      $.ajax(SCRIPT_HOST + '/api/pref/' + $.map(locations, encodeURIComponent).join('/') + '/lines', {
        cache: true,
        dataType: 'jsonp'
      })
        .done(function(data) {
          self.update_lines(data.lines);
    ...
    on_change: function(changed) {
      ...
      $.ajax(SCRIPT_HOST + '/api/area/' + $.map(locations, encodeURIComponent).join('/'), {
        ...
        .done(function(data) {
          ...
          if (data.areas.indexOf(decodeURIComponent(params['address_city'])) >= 0)
            $target.val(decodeURIComponent(params['address_city'])).trigger('change');
        })
        .fail(function() {
          // TODO
        });
