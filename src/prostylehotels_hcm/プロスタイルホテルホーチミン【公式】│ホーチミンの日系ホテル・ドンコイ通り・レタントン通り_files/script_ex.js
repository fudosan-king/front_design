$(document).ready(function() {
    ...
    // Accommodations
	$('.box_walking, .box_walkinghere').click(function(event) {
		/* Act on the event */
		var idScroll = $(this).attr('data-walk');
		if ($("#" + idScroll).length){
		 	$('html, body').animate({
    ...
    // $('.accommodation-link').click(function(event) {
    ...
    $('.bsnav-mobile .nav-link').click(function(event) {
        /* Act on the event */
        // event.preventDefault();
        console.log($(this).hasClass('btnLang'));
        if(!$(this).hasClass('btnLang')){
            $bsnav = $(this).closest('.bsnav-mobile');
            if($('.bsnav-mobile.in').length)
                $bsnav.find('.navbar-toggler').click();
        }
    });
