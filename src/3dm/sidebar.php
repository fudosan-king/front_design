<div id="sidebar-container" class="sidebar-expanded d-none d-md-block">
    <ul class="list-group sidebar">
        <li>
            <a href="" class="list-group-item">ダッシュボード</a>
        </li>
        <li class="active">
            <a href="index.php" class="list-group-item">図面</a>
        </li>
        <li>
            <a href="list_item.php" class="list-group-item">アイテム</a>
        </li>
        <li>
            <a href="category.php" class="list-group-item">カテゴリー</a>
        </li>
        <li>
            <a href="texture.php" class="list-group-item">テクスチャ</a>
        </li>
        <li>
            <a href="" class="list-group-item">ユーザー</a>
        </li>
        <li>
            <a href="" class="list-group-item">権限管理</a>
        </li>
        <li>
            <a href="" class="list-group-item">インポート</a>
        </li>
        <li>
            <a href="" class="list-group-item">設定</a>
        </li>
    </ul>
</div>