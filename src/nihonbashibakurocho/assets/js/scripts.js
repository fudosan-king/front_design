...
$(function() {
	...
	var fullpage = $('#fullpage'), flg = false, body = $('body'), flg_fullpage = false,
			fullpage_args = {
				...
				afterLoad: function(anchorLink, index) {
					switch (index) {
						case 2:
							$('.section00 .txt').delay(0).fadeIn(3000);
							break;
						case 3:
							$('.section06 ul').delay(600).fadeIn(1500);
							break;
						case 4:
	...
	var func_main_visual_switcher = function() {
		...
		$.each(bg_images, function(i) {
			...
			if (0 < main_visual.find('.fp-tableCell').length) {
				main_visual.find('.fp-tableCell').append('<div id="layer' + (i + 1)  + '" class="layer' + ac +'" style="background-image: url(' + this + ')"><img src="assets/images/top/slide_catch' + (i + 1)  + '.png"></div>');
			} else {
				main_visual.append('<div id="layer' + (i + 1)  + '" class="layer' + ac +'" style="background-image: url(' + this + ')"><img class="catch" src="assets/images/top/slide_catch' + (i + 1)  + '.png"><p class="price"><img src="assets/images/top/slide_price_171116.png"></p></div>');
			}
		});
		...
		function bg_view() {
			...
			timer = setInterval(function() {
				...
				if (cnt === layer.length) {
					...
					$('.layer').delay(1500).fadeOut(1200, function() {
					});*/
					flg = true;
					loop = true;
				...
				/*layer.eq(cnt - 1).delay(2000).removeClass('active');
				layer.eq(cnt).delay(2000).addClass('active');
	...
	var func_main_visual_height = function() {
		...
		if (0 < mv.length) {
			$(window).on('load resize', function() {
				...
				if (767 < w) {
					mv.css('paddingTop', '77px');
				} else {
					mv.css('paddingTop', (((h - 56) / w) * 100) + '%');
				}
			});
		}
	...
	function smp_scroll() {
		...
		if ( $('.bn-mv-modelroom').length ) {
			bn =  $('.bn-mv-modelroom'),
			bn_pos = (bn.length ) ? bn.position().top : 0;
		}
