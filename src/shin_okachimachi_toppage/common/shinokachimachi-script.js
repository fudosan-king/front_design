function shinokachimachi() {
    ...
    this.ready = function() {
        ...
        $(document).ready(function() {
            ...
            _this.buttonClick();
            _this.popupHome();
            // _this.buttonRadioInquiry();
            // _this.menuInquiry();
            // _this.hiddenDateTime();
        });
    }

    this.popupHome = function () {
        $('.popup-close').on('click',function(e){
            $(e.currentTarget).closest('.popup-block').toggle();
        });
    }
    ...
    this.planLink = function() {
        $('body').off('click', '.kurame-plan-item').on('click', '.kurame-plan-item', function(e){
            e.preventDefault();
            let current = $(e.currentTarget);
             let link = current.attr('data-plan-link');
             if(link) {
    ...
    this.buttonClick = function() {
        $('#button_order').click(function (e) {
            if (this.id == 'button_order') {
                let current = $(e.currentTarget);
                let data_order = current.attr('data_button_order');
                data_radio = data_order
                sessionStorage.setItem('btn_radio', data_order);
            }
         });

         $('#btn_reservation').click(function (e) {
            if (this.id == 'btn_reservation') {
                let current = $(e.currentTarget);
                let data_reservation = current.attr('data_button_reservation');
                data_radio = data_reservation;
                sessionStorage.setItem('btn_radio', data_reservation);
            }
         });
    }

    this.buttonRadioInquiry = function() {

        let button_radio = sessionStorage.getItem('btn_radio');
        let pageInquiry = document.getElementById('inquiry');
        
        if (pageInquiry) {
            let currentRequest = null;
            if(button_radio == document.getElementById('request_document1').defaultValue ) {
                let requestItem1 = $('#request_document1');
                if (requestItem1.length) {
                    requestItem1.attr('checked', true);
                    currentRequest = requestItem1;
                }
            } else {
                let requestItem2 = $('#request_document2');
                if (requestItem2.length) {
                    requestItem2.attr('checked', true);
                    currentRequest = requestItem2;
                }
            }
            if (currentRequest) {
                let requestEle = currentRequest.closest('form').find('input[name="request_document"]');
                requestEle.val(currentRequest.val());
            }
        }
        
    }

    this.menuInquiry = function() {

        $('#資料請求').click(function (e) {
            if (this.id == '資料請求') {
                sessionStorage.setItem('btn_radio', this.id);
            }
         });

         $('#来場予約').click(function (e) {
            if (this.id == '来場予約') {
                sessionStorage.setItem('btn_radio', this.id);
            }
         });
         ...
         if(pageInquiry) {
            ...
            if(button_radio == document.getElementById('request_document1').defaultValue ) {
                document.getElementById('request_document1').checked = true;
            } else {
                document.getElementById('request_document2').checked = true;
            }
         }
         
    }

    this.hiddenDateTime = function() {
        let checkedValue = $('input[name="request_document[]"]:checked').val();
        
        if (checkedValue == '資料請求') {
        ...
        $('input[name="request_document[]"]').on('click', function(event) {
            checkedValue = $(event.currentTarget).val();
