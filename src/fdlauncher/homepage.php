<!doctype html>
<html class="no-js" lang="jp">

<head>
    <?php require 'head.php'; ?>
</head>

<body>
    <div id="page">
        <?php include 'header.php'; ?>

        <section class="section_banner">
            <img class="img-fluid img_banner" src="assets/images/banner_img.svg" alt="banner images" data-aos="fade-down">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="banner_content">
                            <div class="box_info" data-aos="fade-up">
                                <h2>
                                    <span>スタートアップ不動産企業に最適</span><br>
                                    不動産物件コンバーター
                                </h2>
                                <h3>FDランチャー</h3>
                                <ul>
                                    <li>
                                        <p>初期費用</p>
                                        <p><span>0</span>円</p>
                                    </li>
                                    <li>
                                        <p>月額費用</p>
                                        <p><span>1</span>万円</p>
                                    </li>
                                    <li>
                                        <p>掲載物件数</p>
                                        <p>無制限</p>
                                    </li>
                                </ul>
                                <a target="_blank" href="https://form.run/@fdl-document-request-form" class="btn btn_requestinquiries butn__new"><i></i> 資料請求・お問合せ</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_features" class="section_features">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <h2>FDランチャーの６つの特徴</h2>
                        <ul>
                            <li>
                                <h3>SEO、LPO対策</h3>
                                <img src="assets/images/send_mail.svg" alt="send email" width="256">
                                <p>WEBサイトでの検索結果の上位表示に必要な設定が簡単にできます。</p>
                            </li>
                            <li>
                                <h3>物件検索機能</h3>
                                <img src="assets/images/img_map.svg" alt="map" width="242">
                                <p>エリアや沿線、駅名からの検索はもちろん、価格や間取りなどの検索にも対応します。</p>
                            </li>
                            <li>
                                <h3>テンプレートで簡単作成</h3>
                                <img src="assets/images/img_images.svg" alt="send email" width="186">
                                <p>テンプレート設計だから管理画面もシンプルでわかりやすい。<br>
                                    バナーの変更や物件更新も簡単に行えます。ホームページの立ち上げをフォローします。</p>
                            </li>

                            <li>
                                <h3>スマホにも対応</h3>
                                <img src="assets/images/img_idea.svg" alt="send email" width="244">
                                <p>パソコンだけでなくスマートフォンやタブレットから見たときにもきちんとホームページが表示される「レスポンシブ」にも対応します。</p>
                            </li>
                            <li>
                                <h3>物件コンバーター連動</h3>
                                <img src="assets/images/img_linked.svg" alt="linked" width="315">
                                <p>物件入稿コンバーターと連携しているので自社ホームページや主要ポータルサイト（スーモ、アットホーム、ホームズ）にも簡単に物件を一括で入稿できます。</p>
                            </li>
                            <li>
                                <h3>フォロー体制</h3>
                                <img src="assets/images/img_chart.svg" alt="chart" width="239">
                                <p>公開後もWEB広告や分析方法など集客の基礎作りを支援します。（一部有償オプション）</p>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_price" class="section_price">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="price_top" data-aos="fade-up">
                            <h2 class="title">PRICE</h2>
                            <h3 class="sub_title">圧倒的な低価格</h3>
                            <p class="description">テンプレートでホームページが簡単に制作でき、コンバート機能もついて、月額たったの11,000円。<br>
                                しかもアカウントユーザー数の制限もありません。</p>
                            <img class="img-fluid img_stmp d-none d-lg-block" src="assets/images/STMP.svg" alt="STMP">
                        </div>
                        <div class="table-responsive w-100" data-aos="fade-up">
                            <table class="table table-bordered">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <th>A社</th>
                                        <th>B社</th>
                                        <th>当社</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td>初期費用</td>
                                        <td>￥165,000</td>
                                        <td>￥440,000</td>
                                        <td>¥0</td>
                                    </tr>
                                    <tr>
                                        <td>月額費用</td>
                                        <td>¥22,000</td>
                                        <td>￥27,500</td>
                                        <td>¥11,000※2</td>
                                    </tr>
                                    <tr>
                                        <td class="fw-normal">ホームページデザイン</td>
                                        <td class="fw-normal">カスタムテンプレート</td>
                                        <td class="fw-normal">オリジナル</td>
                                        <td class="fw-normal">テンプレート</td>
                                    </tr>
                                    <tr>
                                        <td>コンバーター初期費用</td>
                                        <td>￥55,000</td>
                                        <td>機能なし</td>
                                        <td>¥0</td>
                                    </tr>
                                    <tr>
                                        <td>コンバーター月額費用</td>
                                        <td>￥21,780</td>
                                        <td>機能なし</td>
                                        <td>¥0</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <p class="text-end">
                            <small>※1  A社と比較した場合の初年度合計額<br>
                                ※2  API仕様の場合、月額費用¥16,500</small>
                        </p>
                    </div>
                </div>
            </div>
        </section>

        <!-- <section id="section_submissiontool" class="section_submissiontool">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="box_submissiontool">
                            <div class="box_submissiontool_info" data-aos="fade-left">
                                <h2>物件情報一括入稿ツール</h2>
                                <h3>FDランチャー</h3>
                                <p>FDランチャーは、 主要ポータルサイトに「一度の手間」で一気に広告を拡散するシステムです。各主要ポータルサイトと連動しており、低コストかつわかりやすい操作性で、人を増やさずとも、貴社の生産力が一気に上がります。</p>
                                <a href="#" class="btn">
                                    <span>月額費用</span>
                                    <span>10,000</span>円 <i>（税抜）</i></a>
                            </div>
                            <img class="img-fluid" src="assets/images/submissiontool.svg" alt="submissiontool" data-aos="fade-right">
                        </div>
                    </div>
                </div>
            </div>
        </section> -->

        <section id="section_plan" class="section_plan">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="plan_top" data-aos="fade-up">
                            <h2 class="title">PLAN</h2>
                            <h3 class="sub_title">自社サイトの物件情報も同時更新</h3>
                        </div>
                        <ul>
                            <li data-aos="fade-right">
                                <h4>これからホームページを作る方</h4>
                                <a href="#" class="btn">テンプレートで簡単作成</a>
                                <h5>専門知識がなくてもホームページが簡単に作成できます。お問い合わせフォームの追加も可能です。</h5>
                                <img src="assets/images/templates.svg" alt="templates">
                                <p>入力フォームにデータを入力するだけでホームページが作成できます。難しい知識は必要ありません。入力した物件情報もそのままホームページに反映されます。</p>
                                <img src="assets/images/arrow_down.svg" alt="arrow_down" width="120">
                                <h6>追加料金無し</h6>
                            </li>
                            <li data-aos="fade-left">
                                <h4>自社ホームページをお持ちの方</h4>
                                <a href="#" class="btn">API活用</a>
                                <h5>貴社ホームページのデザインや機能を損なうことなく、物件情報を更新することができます。</h5>
                                <img src="assets/images/api.svg" alt="templates">
                                <p>FDランチャーのAPI機能を使って、貴社ホームページの物件情報を更新することができます。一度の手間でポータルサイトへの入稿、自社ホームページのデータ更新を行うことができます。</p>
                                <img src="assets/images/arrow_down.svg" alt="arrow_down" width="120">
                                <h6>月額費用 <span>➕</span> <span>5,000</span><i>円</i><i> （税抜）</i></h6>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_propertyinformation" class="section_propertyinformation">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="box_propertyinformation">
                            <div class="box_info" data-aos="fade-right">
                                <h2>物件情報一括入稿ツール</h2>
                                <img class="img-fluid logo_black" src="assets/images/logo_black.svg" alt="logo black" width="290">
                                <a target="_blank" href="https://form.run/@fdl-order-form" class="btn btn_application"><i class="i_edit"></i> 申し込み</a>
                            </div>
                            <img class="img-fluid" src="assets/images/submissiontool_img.svg" alt="logo black" width="453" data-aos="fade-left">
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_flow" class="section_flow">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="top_flow" data-aos="fade-down">
                            <h2 class="title">FLOW</h2>
                            <h3 class="sub_title"><span class="d-block d-lg-inline-block">ご契約から</span>利用開始までの流れ</h3>
                        </div>
                        <img class="img-fluid d-lg-block d-none m-auto" src="assets/images/flow_pc_3.png" alt="flow" data-aos="fade-up">
                        <img class="img-fluid d-block d-lg-none m-auto" src="assets/images/flow_sp_3.png" alt="flow" data-aos="fade-up">
                        <p class="note_flow">※ポータルサイトへの掲載をご希望される場合は、各媒体より一括連動用IDの取得をお願いいたします。</p>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_faq" class="section_faq">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="top_faq" data-aos="fade-down">
                            <h2 class="title">FAQ</h2>
                            <h3 class="sub_title">よくある質問</h3>
                        </div>
                        <div class="accordion" id="accordionFAQ" data-aos="fade-up">
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                                        独自ドメインを使いたいのですが。
                                    </button>
                                </h2>
                                <div id="collapseOne" class="accordion-collapse collapse show" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>独自ドメインもご利用できます。ご相談ください。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                                        登録物件数に制限ありますか？
                                    </button>
                                </h2>
                                <div id="collapseTwo" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>制限はありません。登録物件数に応じて料金が変わることもございません。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                                        各ポータルサイトの検索項目の漏れは無いですか？
                                    </button>
                                </h2>
                                <div id="collapseThree" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>新しく検索項目が追加された場合は、優先順位をつけて随時対応しております。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFour" aria-expanded="false" aria-controls="collapseThree">
                                        自社HPへの連動できますか？
                                    </button>
                                </h2>
                                <div id="collapseFour" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>可能です。詳細はお問合せください。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFive" aria-expanded="false" aria-controls="collapseThree">
                                        自社HPへ連動する場合、運用までにどのくらい時間がかかりますか？
                                    </button>
                                </h2>
                                <div id="collapseFive" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>HPの内容によって期間は前後しますので、詳細なスケジュールはお打ち合わせ後にお伝えいたします。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSix" aria-expanded="false" aria-controls="collapseThree">
                                        月額費用10,000円 （税抜）以外にかかる費用はありますか？
                                    </button>
                                </h2>
                                <div id="collapseSix" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>不動産ポータルサイトといくつ連動しても初期費用0円、月額費用10,000円（税抜）です。<br>
                                            ただし、貴社と各不動産ポータルサイトで利用契約を結ぶときに別途費用が掛かります。<br>
                                            また、既存自社サイトと連動する場合は月額費用15,000円（税抜）となります。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSeven" aria-expanded="false" aria-controls="collapseThree">
                                        後から連動ポータルサイトを増やすことはできますか？
                                    </button>
                                </h2>
                                <div id="collapseSeven" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>可能です。追加料金などはいただきません。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEight" aria-expanded="false" aria-controls="collapseThree">
                                        賃貸に対応していますか？
                                    </button>
                                </h2>
                                <div id="collapseEight" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>現状、『売買仲介のみ』の対応となっております。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseNine" aria-expanded="false" aria-controls="collapseThree">
                                        どの程度の規模の会社が導入していますか？
                                    </button>
                                </h2>
                                <div id="collapseNine" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>1店舗からご利用いただけます。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTen" aria-expanded="false" aria-controls="collapseThree">
                                        申し込んでから、どれくらいで利用できるようになりますか？
                                    </button>
                                </h2>
                                <div id="collapseTen" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>一括入稿システムに関しては各ポータルサイトの契約状況により異なりますが、2週間〜1ヶ月ほどでご利用できます。</p>
                                    </div>
                                </div>
                            </div>
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseEleven" aria-expanded="false" aria-controls="collapseThree">
                                        インボイスに対応していますか？
                                    </button>
                                </h2>
                                <div id="collapseEleven" class="accordion-collapse collapse" data-bs-parent="#accordionFAQ">
                                    <div class="accordion-body">
                                        <p>対応しています。</p>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_topics" class="section_topics">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="top_topics" data-aos="fade-down">
                            <h2 class="title">TOPICS</h2>
                            <h3 class="sub_title">追加予定機能</h3>
                        </div>
                        <div class="topics_content" data-aos="fade-up">
                            <ul>
                                <li><a href="#">ホームページのデザインテンプレートの拡充 (ホームページ機能)</a></li>
                                <li><a href="#">物件取扱エリア拡大（物件コンバーター機能）</a></li>
                                <li><a href="#">物件取扱種別拡大（賃貸）（物件コンバーター機能）</a></li>
                                <li><a href="#">物件情報の共有機能（物件コンバーター機能）</a></li>
                            </ul>
                            <p class="text-end"><a href="#">その他順次開発予定</a></p>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section id="section_propertyinformation" class="section_propertyinformation">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="box_propertyinformation">
                            <div class="box_info" data-aos="fade-right">
                                <h2>物件情報一括入稿ツール</h2>
                                <img class="img-fluid logo_black" src="assets/images/logo_black.svg" alt="logo black" width="290">
                                <a target="_blank" href="https://form.run/@fdl-order-form" class="btn btn_application"><i class="i_edit"></i> 申し込み</a>
                            </div>
                            <img class="img-fluid" src="assets/images/submissiontool_img.svg" alt="logo black" width="453" data-aos="fade-left">
                        </div>
                    </div>
                </div>
            </div>
        </section>


    </div>

    <?php require 'footer.php'; ?>
    <?php require 'js-footer.php'; ?>
</body>

</html>